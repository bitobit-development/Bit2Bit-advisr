  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const leadResponse = await fetch('/api/vehicle-quote', {
          method: 'POST',
          ...
          body: JSON.stringify({
            ...
            lead_type: 'VA - Vehicle Quote',
            id_number: leadInfo.id_number,
            consent: leadInfo.consent,
            discoveryClient: leadInfo.discoveryClient,
          }),
        })
        ...
        if (leadData.success) {
          ...
          const message = `Hi ${firstName}, we've received your vehicle insurance quote request. Your unique reference number is ${uniqueId}. Please keep it safe and mention it to the Vitality team upon arrival. On successful application, you'll stand a chance to win a JBL Go 3 Bluetooth Speaker!\nCarla Prinsloo,\nYour Financial Advisor.`;
          ...
          router.push('/new-vehicle-leads/thank-you')
        } else {
          console.error('❌ Failed to insert lead:', leadData.message)
          router.push(`/new-vehicle-leads/failed?reason=${encodeURIComponent(leadData.details || leadData.message || 'Lead submission failed')}`)
        }
      } else {
